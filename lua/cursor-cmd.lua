...
local ns = vim.api.nvim_create_namespace("cursor-cmd")
...
function M.show(content, pos, firstc, prompt, indent, level, hl_id)
	...
	if line:sub(1, 1) == ':' then right_padding = 1 end

	cmdline_win = vim.api.nvim_open_win(cmdline_buf, false, {
		relative = "cursor",
		row = offset_y,
		col = offset_x,
		width = math.max(#line + right_padding, 2),
		height = 1,
		style = "minimal",
		border = { "", "", "", "", "", "", "", "" },
		zindex = 200,
	...
	if line:sub(1, 1) == '/' then
		vim.wo[cmdline_win].winhighlight = "Normal:CurSearch"
	else 
		vim.wo[cmdline_win].winhighlight = "Normal:NormalFloat"
	end
